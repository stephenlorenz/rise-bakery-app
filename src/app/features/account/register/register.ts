  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-[#EDECE8] px-4">
      <div class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-[#C5CADF] p-8">
        <h1 class="font-serif text-3xl text-[#1E2347] mb-2">Create account</h1>
        <p class="text-[#7279A5] mb-8">Join Rise Bakery to place orders</p>
        ...
        <form (ngSubmit)="submit()" class="space-y-5">
          <div>
            <label class="block text-sm font-medium text-[#1E2347] mb-1">Full name</label>
            <input
              type="text"
              [(ngModel)]="fullName"
              name="fullName"
        ...
        <p class="mt-6 text-center text-sm text-[#7279A5]">
          ...
          <a routerLink="/account/login" class="text-[#4557A7] hover:underline font-medium">Sign in</a>
        </p>
      </div>
    </div>
  `,
