...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto px-4 py-12">
      <h1 class="font-serif text-4xl text-[#1E2347] mb-2">Checkout</h1>

      <!-- Steps indicator -->
      <div class="flex items-center gap-2 mb-10">
        <span [class]="stepClass('pickup')">1. Pickup</span>
        <div class="flex-1 h-px bg-[#C5CADF]"></div>
      ...
      @if (step() === 'pickup') {
        <div class="bg-white rounded-2xl border border-[#C5CADF] p-6 mb-6">
          <h2 class="font-serif text-xl text-[#1E2347] mb-5">Select pickup time</h2>
          <app-pickup-selector (selectionChange)="onPickupSelected($event)" />
        </div>

        <!-- Order summary -->
        <div class="bg-white rounded-2xl border border-[#C5CADF] p-6 mb-6">
          <h2 class="font-serif text-xl text-[#1E2347] mb-4">Order summary</h2>
          @for (item of cart.items(); track item.product.id) {
            <div class="flex justify-between py-2 text-sm border-b border-[#F5EFE6] last:border-0">
              <span class="text-[#1E2347]">{{ item.product.name }} × {{ item.quantity }}</span>
              <span class="text-[#7279A5]">{{ formatPrice(item.product.price_cents * item.quantity) }}</span>
            </div>
          }
          <div class="flex justify-between pt-3 font-semibold">
            <span class="text-[#1E2347]">Total</span>
            <span class="text-[#1E2347]">{{ formatPrice(cart.totalCents()) }}</span>
          </div>
        </div>
      ...
      @if (step() === 'payment' && clientSecret()) {
        <div class="bg-white rounded-2xl border border-[#C5CADF] p-6">
          <h2 class="font-serif text-xl text-[#1E2347] mb-5">Payment</h2>
          @if (pickup()) {
            <p class="text-sm text-[#7279A5] mb-6">
      ...
      @if (step() === 'success') {
        <div class="text-center py-16">
          ...
          <h2 class="font-serif text-3xl text-[#1E2347] mb-3">Order confirmed!</h2>
          <p class="text-[#7279A5] mb-8">
            Your order is confirmed for pickup on {{ formatDate(pickup()?.date ?? '') }} at {{ pickup()?.time }}.
          </p>
          <a routerLink="/orders" class="bg-[#4557A7] text-white px-8 py-3 rounded-lg font-medium hover:bg-[#374899] transition-colors">
            View my orders
          </a>
        </div>
      }
...
export class CheckoutComponent {
  ...
  stepClass(s: Step): string {
    ...
    if (active) return 'text-[#4557A7] font-medium text-sm';
    if (done) return 'text-[#7279A5] text-sm line-through';
    return 'text-[#7279A5] text-sm';
  }
