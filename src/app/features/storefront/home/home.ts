  ...
  template: `
    ...
    <section class="relative h-[70vh] min-h-[480px] flex items-center justify-center overflow-hidden bg-[#1E2347]">
      ...
      <div class="relative z-20 text-center px-4">
        ...
        <p class="text-[#C5CADF] text-lg mb-8 max-w-md mx-auto">
          Artisan bread loaves baked weekly. Place your order and pick up fresh.
        </p>
        <a routerLink="/menu"
           class="inline-block bg-[#4557A7] hover:bg-[#374899] text-white font-medium px-8 py-4 rounded-full transition-colors text-lg">
          Shop the Menu
        </a>
      </div>
    </section>
    ...
    <section class="max-w-6xl mx-auto px-4 py-16">
      <div class="text-center mb-12">
        <h2 class="font-serif text-4xl text-[#1E2347] mb-3">This Week's Loaves</h2>
        <p class="text-[#7279A5]">Limited quantities — order early</p>
      </div>
      ...
      @if (loading()) {
          ...
          @for (i of [1,2,3]; track i) {
            <div class="bg-white rounded-2xl overflow-hidden border border-[#C5CADF] animate-pulse">
              ...
              <div class="p-5 space-y-3">
                ...
                <div class="h-4 bg-[#C5CADF] rounded w-3/4"></div>
              </div>
            </div>
          }
        </div>
        ...
        @if (featuredProducts().length > 0) {
          <div class="text-center mt-10">
            <a routerLink="/menu" class="text-[#4557A7] hover:underline font-medium">
              View full menu →
            </a>
          </div>
        }
    ...
    <!-- How it works -->
    <section class="bg-[#1E2347] py-16 px-4">
      <div class="max-w-4xl mx-auto text-center">
        <h2 class="font-serif text-4xl text-white mb-12">How it works</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          @for (step of steps; track step.num) {
            <div>
              <div class="w-12 h-12 rounded-full bg-[#4557A7] text-white font-serif text-xl flex items-center justify-center mx-auto mb-4">
                {{ step.num }}
              </div>
              <h3 class="font-serif text-xl text-white mb-2">{{ step.title }}</h3>
              <p class="text-[#7279A5] text-sm">{{ step.desc }}</p>
            </div>
          }
        </div>
      </div>
