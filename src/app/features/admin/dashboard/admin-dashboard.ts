...
@Component({
  ...
  template: `
    <div class="flex min-h-screen bg-[#FAF7F2]">
      <!-- Sidebar -->
      <aside class="w-60 bg-[#3E2723] text-[#E8D5B7] flex flex-col shrink-0">
        <div class="p-6 border-b border-[#5D3A35]">
          <p class="font-serif text-xl text-white">Rise Bakery</p>
          <p class="text-xs text-[#8D7B68] mt-1">Admin Panel</p>
        </div>
        <nav class="flex-1 p-4 space-y-1">
          @for (link of navLinks; track link.path) {
            <a
              [routerLink]="link.path"
              routerLinkActive="bg-[#B85C38] text-white"
        ...
        <div class="p-4 border-t border-[#5D3A35]">
          <a routerLink="/" class="text-xs text-[#8D7B68] hover:text-white">← Back to store</a>
        </div>
      </aside>
